...
#define DISTRIBUTION_SHORT_NAME "exponential"
#define DISTRIBUTION_TEXT       string(string(DISTRIBUTION_SHORT_NAME) + " distribution")
#define DISTRIBUTION_NAME       exponential_distribution
...
struct distribution_traits_base {
	...
	static constexpr std::array<const char *, 1> param_names = {"rate"};
	...
	static void ValidateParameters(param1_t rate) {
		if (rate <= 0) {
			throw InvalidInputException(string(DISTRIBUTION_SHORT_NAME) +
			                            ": Rate must be positive was: " + std::to_string(rate));
		}
	}
};
