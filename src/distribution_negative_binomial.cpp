...
#define DISTRIBUTION_SHORT_NAME "negative_binomial"
#define DISTRIBUTION_TEXT       string(string(DISTRIBUTION_SHORT_NAME) + " distribution")
#define DISTRIBUTION_NAME       negative_binomial_distribution
...
struct distribution_traits_base {
	...
	static constexpr std::array<const char *, 2> param_names = {"successes", "prob"};
	...
	static void ValidateParameters(param1_t successes, param2_t prob) {
		if (successes <= 0) {
			throw InvalidInputException(string(DISTRIBUTION_SHORT_NAME) +
			                            ": Number of successes must be > 0 was: " + std::to_string(successes));
		}
