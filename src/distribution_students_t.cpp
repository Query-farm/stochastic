...
#define DISTRIBUTION_SHORT_NAME "students_t"
#define DISTRIBUTION_TEXT       string(string(DISTRIBUTION_SHORT_NAME) + " distribution")
#define DISTRIBUTION_NAME       students_t_distribution

#define DISTRIBUTION        boost::math::DISTRIBUTION_NAME<double>
#define SAMPLE_DISTRIBUTION boost::random::student_t_distribution<double>
...
struct distribution_traits_base {
	...
	static constexpr std::array<const char *, 1> param_names = {"degrees_of_freedom"};
	static constexpr const char *prefix = DISTRIBUTION_SHORT_NAME;

	static std::vector<LogicalType> LogicalParamTypes() {
		return {logical_type_map<param1_t>::Get()};
	}

	static void ValidateParameters(param1_t degrees_of_freedom) {
		if (degrees_of_freedom <= 0) {
			throw InvalidInputException(
			    string(DISTRIBUTION_SHORT_NAME) +
			    ": Degrees of freedom must be positive was: " + std::to_string(degrees_of_freedom));
		}
	}
};
...
LOAD_DISTRIBUTION_FN {
	...
	auto make_unary = [](auto func) {
		return [func](DataChunk &args, ExpressionState &state, Vector &result) {
			DistributionCallUnaryUnary<DISTRIBUTION, double>(args, state, result, func);
		};
	};

	auto make_none = [](auto func) {
		return [func](DataChunk &args, ExpressionState &state, Vector &result) {
			DistributionCallUnaryNone<DISTRIBUTION>(args, state, result,
	...
	REGISTER(
	    ...
	    [](DataChunk &args, ExpressionState &state, Vector &result) {
		    DistributionSampleUnary<SAMPLE_DISTRIBUTION, double>(args, state, result);
	    },
	    "Generates random samples from the " + DISTRIBUTION_TEXT + " with specified parameters.", "sample(10)");
	...
	REGISTER(
	    ...
	    "log_pdf(10, 1.5)", param_names_unary);
	...
	REGISTER(loader, "cdf_complement", FunctionStability::CONSISTENT, LogicalType::DOUBLE,
	         ...
	         "cdf_complement(10, 1.5)", param_names_unary);
	...
	REGISTER(
	    ...
	    "log_cdf(10, 1.5)", param_names_unary);
	...
	REGISTER(loader, "log_cdf_complement", FunctionStability::CONSISTENT, LogicalType::DOUBLE,
	         ...
	         "log_cdf_complement(10, 1.5)", param_names_unary);

	// === QUANTILE FUNCTIONS ===
	REGISTER(
	    loader, "quantile", FunctionStability::CONSISTENT, LogicalType::DOUBLE,
	    ...
	    "quantile(10, 0.95)", param_names_quantile);
	...
	REGISTER(loader, "quantile_complement", FunctionStability::CONSISTENT, LogicalType::DOUBLE,
	         ...
	         "quantile_complement(10, 0.05)", param_names_quantile);
	...
	REGISTER(
	    ...
	    "Computes the hazard function of the " + DISTRIBUTION_TEXT + ".", "hazard(10, 1.5)", param_names_unary);
	...
	REGISTER(loader, "chf", FunctionStability::CONSISTENT, LogicalType::DOUBLE,
	         ...
	         "Computes the cumulative hazard function of the " + DISTRIBUTION_TEXT + ".", "chf(10, 1.5)",
	         param_names_unary);
	...
	REGISTER(loader, "mean", FunctionStability::CONSISTENT, LogicalType::DOUBLE,
	         ...
	         "Returns the mean (μ) of the " + DISTRIBUTION_TEXT + ", which is the first moment.", "mean(10)");
	...
	REGISTER(loader, "stddev", FunctionStability::CONSISTENT, LogicalType::DOUBLE,
	         ...
	         "Returns the standard deviation (σ) of the " + DISTRIBUTION_TEXT + ".", "stddev(10)");
	...
	REGISTER(loader, "variance", FunctionStability::CONSISTENT, LogicalType::DOUBLE,
	         ...
	         "Returns the variance (σ²) of the " + DISTRIBUTION_TEXT + ".", "variance(10)");
	...
	REGISTER(loader, "skewness", FunctionStability::CONSISTENT, LogicalType::DOUBLE,
	         ...
	         "Returns the skewness of the " + DISTRIBUTION_TEXT + ".", "skewness(10)");
	...
	REGISTER(loader, "kurtosis", FunctionStability::CONSISTENT, LogicalType::DOUBLE,
	         ...
	         "Returns the kurtosis of the " + DISTRIBUTION_TEXT + ".", "kurtosis(10)");
	...
	REGISTER(loader, "kurtosis_excess", FunctionStability::CONSISTENT, LogicalType::DOUBLE,
	         ...
	         "Returns the excess kurtosis of the " + DISTRIBUTION_TEXT + ".", "kurtosis_excess(10)");
	...
	REGISTER(loader, "range", FunctionStability::CONSISTENT, LogicalType::ARRAY(LogicalType::DOUBLE, 2),
	         ...
	         "Returns the range of the " + DISTRIBUTION_TEXT + ".", "range(10)");
	...
	REGISTER(loader, "support", FunctionStability::CONSISTENT, LogicalType::ARRAY(LogicalType::DOUBLE, 2),
	         ...
	         "Returns the support of the " + DISTRIBUTION_TEXT + ".", "support(10)");
}
